...
- **Standalone field-test Android build installs alongside the dev client, as a separate app.** `android/app/build.gradle`'s `release` buildType has `applicationIdSuffix ".field"` + `versionNameSuffix "-field"`, and `android/app/src/release/res/values/strings.xml` overrides `app_name` to "SparkQuote (Field)" — so `./gradlew assembleRelease` (see local build workflow below) produces `com.dqm79.sparkquoteapp.field`, installable via `adb install` without touching the dev-client install (`com.dqm79.sparkquoteapp`). It's a genuinely separate app/package, so it starts with empty WatermelonDB data — no shared state with the dev client. **These files live under the gitignored `android/` folder and are NOT committed** — if `expo prebuild` is ever rerun (new native dependency, `app.json` native-config change), this customization is wiped and must be reapplied by hand (added 2026-07-23).
